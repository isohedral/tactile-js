	...
	if( show_controls ) {
		...
		for( let i of tiling.parts() ) {
			...
			for( let idx = 1; idx < ej.length; ++idx ) {
				...
				if( currentTiling.ptdist( Q, pt ) < 0.5 * phys_unit ) {
					u_constrain = false;
					if( idx == (ej.length-1) ) {
					...
					if( !u_constrain ) {
						delete_timer = setTimeout( currentTiling.deleteVertex, 1000 );
					}
				...
				// Check segment
				if( currentTiling.distToSeg( pt, P, Q ) < 20 ) {
					mode = MODE_MOVE_VERTEX;
					max_touches = 1;
					drag_edge_shape = id;
					drag_vertex = idx;
...
function getTouchRigid()
{
	...
	if( num_touches == 1 ) {
		...
		const M = Tactile.mul( M2, currentTiling.inv( M1 ) );
...
function getTilingRect()
{
	...
	const tiling = currentTiling.getCurrentTiling();
	const t1l = currentTiling.len( tiling.getT1() );
	const t2l = currentTiling.len( tiling.getT2() );
	...
	const ret = currentTiling.getTilingRect(pts, margin);
...
function doTouchMoved()
{
	...
	if( mode == MODE_ADJ_TILING ) {
		...
		tiling_iT = currentTiling.inv( tiling_T );
		loop();
		return false;
...
function draw()
{
	...
	const the_type = currentTiling.getTileType();
